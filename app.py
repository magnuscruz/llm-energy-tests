...
# Handle both local and HF Spaces environments
if os.path.exists(os.path.expanduser("~/git/llm-energy-tests/logs/")):
...
if not os.path.exists(base_path):
    ...
    1. Run benchmarks: `sudo ./scripts/run_benchmark.sh 3600`
    2. Data will be saved to `logs/YYYY-MM-DD/`
...
if not available_dates:
    ...
    st.info("Run `sudo ./scripts/run_benchmark.sh 3600` to generate test data")
    st.stop()
...
if selected_dates:
    ...
    if not df_inf.empty:
        ...
        with col4:
            if not df_phys.empty and 'Watts' in df_phys.columns:
                st.markdown("<div class='plot-container'>", unsafe_allow_html=True)
